...
const depositOptions = [
  { amount: 30, label: 'R$ 30,00', subtext: '(mínimo)' },
  { amount: 50, label: 'R$ 50,00', subtext: '' },
  { amount: 100, label: 'R$ 100,00', subtext: '' },
];
      ...
      <main className="flex-1 w-full max-w-3xl mx-auto py-12 px-4">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-12">
          {depositOptions.map((option) => (
            <Card key={option.amount} className="flex flex-col text-center shadow-lg hover:border-accent transition-all">
              <CardHeader>
                <CardTitle className="text-4xl font-bold">{option.label}</CardTitle>
                {option.subtext && <CardDescription>{option.subtext}</CardDescription>}
              </CardHeader>
